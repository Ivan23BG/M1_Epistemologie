  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.41\textwidth}
      \begin{tikzpicture}[scale=0.85, font=\small]
        \draw[-{Stealth}, thick, umbleu] (0,0) -- (0,4.5) node[above]{\footnotesize capacité};
        \draw[-{Stealth}, thick, umbleu] (0,0) -- (5,0) node[right]{\footnotesize temps};
        % Courbe anciens systèmes
        \draw[thick, gray, dashed] plot[smooth] coordinates
          {(0.2,0.3)(1,0.6)(2,1.0)(2.8,1.2)(3.5,1.3)};
        \node[gray, align=left, font=\tiny] at (1.8,1.55)
          {systèmes experts\\tuteurs adaptatifs};
        % Courbe IAG
        \draw[thick, accent, very thick] plot[smooth] coordinates
          {(3.5,1.3)(3.8,1.5)(4.1,2.5)(4.4,4.0)};
        \node[accent, font=\tiny\bfseries] at (4.55,3.2) {IAG};
        % Flèche 2022
        \draw[dotted] (3.5,0) -- (3.5,1.3);
        \node[font=\tiny] at (3.5,-0.25) {2022};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Cadre théorique : deux piliers}
  ...
  \vspace{0.8em}
  \begin{itemize}
    \item[$\Rightarrow$] La crise créée par les IAG est \textbf{structurelle}, pas accidentelle
    \item[$\Rightarrow$] C'est aussi une crise de \textbf{légitimité}, pas seulement d'efficacité
  \end{itemize}
\end{frame}
...
\begin{frame}{Tension 4 --- La crise des critères (point central)}
  \begin{block}{Le contrat didactique perverti}
    Les évaluations récompensaient la conformité formelle
    plus que la compétence réelle --- les IAG portent
    cette logique à son \textbf{point de rupture}.
  \end{block}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{L'analogie frappante :}\\[0.3em]
      ...
      → L'apprenant et l'IAG partagent
...
\begin{frame}{Ce que les tensions impliquent}
  ...
  \begin{alertblock}{Le diagnostic d'Alexandre (2019) --- à prendre au sérieux}
...
\begin{frame}{Conclusion}
  ...
  \begin{itemize}
    ...
    \item \textbf{Réussite} : le défi n'est pas de restaurer une méritocratie
          menacée, mais de redéfinir ce que l'école veut certifier.
...
\begin{frame}{Conclusion}
  ...
  \begin{alertblock}{Ouverture}
    De l'écriture à l'imprimerie, de la calculatrice à Internet ---
    chaque outil a contraint l'école à redéfinir ce qu'elle cultive.
    L'avenir de l'évaluation est dans l'invention de formes capables
    de saisir ce que les outils \textbf{ne peuvent pas faire à notre place} :
    jugement situé, réflexivité, engagement éthique avec le savoir.
  \end{alertblock}
\end{frame}
